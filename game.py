...
def print_menu(exits):
    ...
    for direction, room in exits.items():
        destination = exit_leads_to(exits, direction)
        print(f"Go {direction.upper()} to {destination}.") 
    print("Where do you want to go?")

def exit_leads_to(exits, direction):
    pass

def print_menu_line(direction, room):
    pass

exits = {
    "EAST": "Tutor's office",
    "WEST": "Parking",
    "SOUTH": "MJ and Simon's room"
}
...
def menu(exits):
    ...
    while True:
        ...
        if is_valid_exit(choice, exits):
            return choice
        
        print("Invalid choice. Please choose a valid exit.")

    def is_valid_exit(choice, exits):

        return choice in exits
    ...
    chosen_exit = menu(rooms["Reception"])
    print(f"You chose to go {chosen_exit}.")
        # COMPLETE THIS PART:
...
if __name__ == "__main__":
    import doctest #not format requested from exercise
    doctest.testmod()
    main()
